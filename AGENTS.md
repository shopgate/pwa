...
- **Read component tokens as vars, so a subtree can be restyled.** A component that renders
  ...
  draws `components.iconButton.boxShadow`, which defaults to `none`; the product header cta row
  redeclares it on the element carrying `theme__product__header__cta-buttons`, and merchant CSS
  overrides that same element (equal specificity, `theme.css` later in the cascade). Resolving the
  token to a literal on a container instead would break this: custom properties substitute at
  computed-value time, so a `--a: var(--b)` on an ancestor freezes against that ancestor's `--b` and
  a later override of `--b` no longer reaches it.
