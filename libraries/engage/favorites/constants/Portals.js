// @flow
import { FAVORITES } from '@shopgate/pwa-common-commerce/favorites/constants/Portals';

export const FAVORITES_LIST = `${FAVORITES}.list`;
export const FAVORITES_LIST_ADD_BUTTON = `${FAVORITES}.list-add-button`;
export const FAVORITES_LIST_CONTEXT_MENU = `${FAVORITES}.list-context-menu`;
export const FAVORITES_LIST_ITEM_ACTIONS = `${FAVORITES}.list-item-actions`;
export const FAVORITES_LIST_ITEM = `${FAVORITES}.list-item`;
export const FAVORITES_NOTES = `${FAVORITES}.notes`;
export const FAVORITES_QUANTITY = `${FAVORITES}.quantity`;
