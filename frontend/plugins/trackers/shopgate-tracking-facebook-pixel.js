...
class SgFbPixelTracking extends SgTrackingPlugin {
  /**
  ...
  initPlugin() {
    ...
    SgFbPixelTracking.sendToFb(firstPixel.toString(), undefined, 'init');

    // add multiple pixel Ids.
    // Warning: This is not official supported by facebook, but seems to work
    pixelsForInit.forEach((pixel) => {
      SgFbPixelTracking.sendToFb(pixel.toString(), undefined, 'addPixelId');
    });
    ...
    SgFbPixelTracking.sendToFb('PageView');

    // register for some events

    this.register.completedRegistration((data) => {
      SgFbPixelTracking.sendToFb('CompleteRegistration', {
    ...
    this.register.viewContent((data, rawData) => {
      ...
      SgFbPixelTracking.sendToFb('ViewContent', fbParams);
    });

    this.register.addedPaymentInfo(() => {
      SgFbPixelTracking.sendToFb('AddPaymentInfo');
    });

    this.register.purchase((data) => {
      const productIds = SgFbPixelTracking.getProductIds(data.items);
      ...
      SgFbPixelTracking.sendToFb('Purchase', fbParams);
    });

    this.register.initiatedCheckout((data, rawData) => {
      const productIds = SgFbPixelTracking.getProductIds(rawData.cart.products);
      ...
      SgFbPixelTracking.sendToFb('InitiateCheckout', fbParams);
    });

    this.register.addToCart((data) => {
      SgFbPixelTracking.sendToFb('AddToCart', this.getParamsForAddtoCartAndWishlist(data));
    });

    this.register.addToWishlist((data) => {
      SgFbPixelTracking.sendToFb('AddToWishlist', this.getParamsForAddtoCartAndWishlist(data));
    });

    this.register.search((data, rawData) => {
      const productIds = SgFbPixelTracking.getProductIds(rawData.products);
      ...
      SgFbPixelTracking.sendToFb('Search', fbParams);
    });
  }
...
// Export the "module"
window.SgFbPixelTracking = SgFbPixelTracking;

export default SgFbPixelTracking;
