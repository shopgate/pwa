...
describe('SgTrackingCore', () => {
  ...
  beforeEach(() => {
    ...
    SgTrackingCore = rerequire('../shopgate-tracking-core').default.reset().registerFinished();
  ...
  it('should make the core globally available in the window object', () => {
    expect(global.window.SgTrackingCore).to.be.equal(SgTrackingCore);
  });
  ...
  it('should handle basic pub/sub functionality', () => {
    ...
    trackableEvents.forEach((event) => {
      ...
      // Track object should be returned to allow chaining
      expect(that).to.equal(SgTrackingCore.track);
  ...
  it('should test getCustomEvents()', () => {
    const expected = [
      'smartbanner',
      'filterLiveSuggest',
      'qrScanner',
    ...
    expect(SgTrackingCore.getCustomEvents()).to.deep.equal(expected);
  });
});
